...
const BankCardInfo = props => {
  ...
  const checkInvalidCard = () => {
    ...
    if (Array.isArray(BinNumbers) && BinNumbers.length > 0) {
      BinNumbers.forEach((element, index) => {
        const prefix = String(element)?.slice(
          element.length - 2,
          element.length,
        );
    ...
    switch (input) {
      case INPUT_REF.CARD_NUMBER: {
        ...
        if (binInvalid) {
          //validate card
          setCardNumberErr(
            'Invalid card number, please start with ' + binInvalid,
          );
        }
        break;
      }
  ...
  const onSubmit = async () => {
    ...
    try {
      const result = await mapBankNapas(BankConnectInfo);
      const {url, OrderId} = result || {};

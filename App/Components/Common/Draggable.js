...
const Draggable = props => {
  ...
  const newPosition = React.useRef({});
  ...
  const onPanResponderRelease = React.useCallback(
    (e, gestureState) => {
      ...
      if (onDragRelease) {
        ...
        onRelease(e, true, newPosition.current, reversePosition);
      }
      if (!shouldReverse) {
  ...
  const handleOnDrag = React.useCallback(
    (e, gestureState) => {
      ...
      newPosition.current = {changeX, changeY};
  ...
  const handlePressOut = React.useCallback(
    event => {
      onPressOut(event, newPosition.current);
      if (!isDragging.current) {
        onRelease(event, false, newPosition.current);
      }
    },
  ...
  return (
    <View pointerEvents="box-none" style={[positionCss]}>
      ...
      <Animated.View
        ...
        style={[pan.current.getLayout(), wrapStyle]}>
        <TouchableOpacity
          {...touchableOpacityProps}
          onLayout={handleOnLayout}
          style={[dragItemCss, itemStyle]}
          disabled={disabled}
